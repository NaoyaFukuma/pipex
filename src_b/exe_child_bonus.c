...
/*   Updated: 2022/09/11 00:52:50 by nfukuma          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	exe_child(int i, t_info *info, char **argv, char **envp)
{
	...
	if (!pid)
	{
		...
		child_argv = ft_split(argv[2 + info->heredoc_flag + i], ' ');
		if (!child_argv)
...
static int	set_child_fd(t_info *info, int i)
{
	...
	{
		if (dup2(info->pipe_fd[2 * i - 2], STDIN) == -1)
			return (-1);
	...
	else
	{
		if (dup2(info->pipe_fd[2 * i - 2], STDIN) == -1)
			return (-1);
		if (dup2(info->pipe_fd[2 * i + 1], STDOUT) == -1)
			return (-1);
	}
	return (0);
}
